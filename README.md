...
Serving: the app must be served via a dev server (`npm run dev`), never opened by double-clicking `index.html` (file:// breaks the audio manifest and API fetches; the app shows a one-line warning). Sound starts only after the "Begin shift" click (browser autoplay rules).
Intel source: the API and `make demo-cli` use the real data/ML/planner modules and train them at startup; `SAATHI_USE_STANDINS=1` switches to the fixture stand-ins.
