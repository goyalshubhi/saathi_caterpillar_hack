...
@needs_node
def test_off_script_extras_are_capped_and_cover_alert_distances():
    ...
    # incident confirmations for all four categories are pre-rendered too
    for lang in ("en", "hi"):
        assert len({e["text"] for e in full if e["message_key"] == "incident_logged" and e["lang"] == lang})             == len(ga.CATEGORIES) == 4
