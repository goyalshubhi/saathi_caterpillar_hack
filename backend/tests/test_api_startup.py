...
def fresh(tmp_path, monkeypatch):
    """Empty synthetic dir + model paths in tmp, nothing cached in memory."""
...
def test_intel_always_uses_the_real_modules():
    # The fixture stand-ins were removed: intel.py calls data_gen / ml / planner directly, no switch.
    from backend import data_gen, ml, planner
    assert (intel.data_gen, intel.ml, intel.planner) == (data_gen, ml, planner)
    assert not hasattr(config, "USE_STANDINS")
