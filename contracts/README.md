...
Conventions: timestamps are ISO 8601 local time without timezone (`2026-09-23T08:00:00`). Minutes are numbers (int or float). Enum values are the exact strings listed.
...
## Weather (`examples/weather_today.json`) — returned by `todays_weather()`
`{date, hourly: [{hour, temperature_c, rain_mm, wind_kmh, condition}]}`. condition uses the weather enum above.
...
## Scenario (`examples/scenario_demo.json`) — returned by `scenario(name)`
`{name, task_id, windows: [TelemetryWindow]}`. Names: `demo` (required), `fatigue` (P1).
...
- message_key = `finding_<type>`. Slots: excessive_idling `{minutes}`, fuel_without_work `{fuel_l}`, unbelted_active `{minutes}`, repeated_alerts `{count}`, fatigue_drift `{}`

## DayPlan (`examples/day_plan.json`)
`{tasks_ordered: [task_id], breaks: [{hour, reason}], condition_warnings: [{task_id, message_key, slots}]}`
- breaks[].reason ∈ routine | heat
- condition_warnings message keys: `warn_rain` `{task_type}`, `warn_wind` `{task_type}`, `warn_heat` `{temperature_c}`
