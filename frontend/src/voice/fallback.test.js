...
describe('fallback voice indicator', () => {
  it("appears when a line's source is 'speech' (no pre-rendered MP3)", () => {
    const { indicator, queue } = wired({ source: 'speech' });
    const seen = [];
  ...
  it('appears for the English fallback and for no voice at all', () => {
    const english = wired({ source: 'speech' }, { fallbackToEnglish: true });
    english.queue.push({ ...LINE, lang: 'hi' });
    expect(english.indicator.state.reason).toBe('english');
    const silent = wired({ source: 'none' });
  ...
  it('is brief: hides after FALLBACK_SHOW_MS, and a new fallback restarts it', () => {
    const { indicator, queue, t } = wired({ source: 'speech' });
    queue.push(LINE);
    queue.push(LINE);
  ...
  it('labels follow the language', () => {
    ...
    indicator.report({ source: 'speech' }, { fallbackToEnglish: false });
